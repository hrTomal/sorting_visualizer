...
root_window.title('Bubble sort visualizer')
...
def initialize_values():
    ...
    if(selected_var.get() == 1):
    	arr = []
    	value = inp_man_num.get().split(',')
    	for i in range(len(value)):
    		arr.append(int(value.pop()))
    	print(arr)
...
def startAlgo():
    ...
    selection_sort(arr, displayBar, speed)
...
rb_rand_val = tkinter.ttk.Radiobutton(f, text='Random Number', variable=selected_var, value=0, command=selected)
...
rb_man_val = tkinter.ttk.Radiobutton(f, text='Manual Number', variable=selected_var, value=1, command=selected)
